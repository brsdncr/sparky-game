...
void PixShader(Input input, inout SurfaceOutput o)
{
	...
	//  & Alpha
	float4 face = _Face;
	float4 outline = _Outline;
	face *= input.;
	outline.a *= input..a;
	face *= tex2D(_FaceTex, float2(input.uv2_FaceTex.x + _FaceUVSpeedX * _Time.y, input.uv2_FaceTex.y + _FaceUVSpeedY * _Time.y));
	outline *= tex2D(_OutlineTex, float2(input.uv2_OutlineTex.x + _OutlineUVSpeedX * _Time.y, input.uv2_OutlineTex.y + _OutlineUVSpeedY * _Time.y));
	face = Get(sd, face, outline, outline, softness);
	face.rgb /= max(face.a, 0.0001);
	...
	bump = lerp(float3(0, 0, 1), bump, face.a);
	n = normalize(n - bump);

	// Cubemap reflection
	fixed4 reflcol = texCUBE(_Cube, reflect(input.viewDirEnv, mul((float3x3)unity_ObjectToWorld, n)));
	float3 emission = reflcol.rgb * lerp(_ReflectFace.rgb, _ReflectOutline.rgb, saturate(sd + outline * 0.5)) * face.a;
#else
