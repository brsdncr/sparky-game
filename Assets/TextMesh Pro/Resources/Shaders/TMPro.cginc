...
fixed4 Get(half d, fixed4 face, fixed4 outline, half outline, half softness)
{
	...
	face = lerp(face, outline, outlineAlpha);
...
float3 GetSpecular(float3 n, float3 l)
{
	...
	return _Specular.rgb * spec * _SpecularPower;
}

float4 GetGlow(float d, float scale)
{
	...
	return float4(_Glow.rgb, saturate(_Glow.a * glow * 2));
}
